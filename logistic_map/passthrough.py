...
class SmallChaoticFlower(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, iter_k=5, alpha=0.1):
        super().__init__()
        self.linear = nn.Linear(input_dim, hidden_dim)
        self.r = nn.Parameter(torch.tensor(3.9))
        self.k = iter_k
        self.alpha = alpha  # eigenchannel strength
        self.output_layer = nn.Linear(hidden_dim, output_dim)

    def logistic_map_with_eigenchannel(self, x):
        for _ in range(self.k):
            x = self.r * x * (1 - x) + self.alpha * x  # <--- eigenchannel passthrough
        return x

    def forward(self, x):
        x = torch.sigmoid(self.linear(x))
        x = self.logistic_map_with_eigenchannel(x)
